...
import { servicesItems } from "~/utils/constant";
import type { ServicesItemsProps } from "~/utils/interface";
const ServiceCard: React.FC<{item: ServicesItemsProps}> = ({item}) => {

  const theme = useMantineTheme()
  const computedColorScheme = useComputedColorScheme('light', { getInitialValueInEffect: true });

  return (
    <Card c={'inherit'} bd={`solid 1px ${computedColorScheme === 'dark' ? theme.colors.blue[7] : theme.colors.blue[1]}`} bg={computedColorScheme === 'dark' ? theme.colors.blue[7] : theme.colors.blue[0]} bdrs={'md'} p={'lg'} shadow="sm" display={'flex'} className="flex-col gap-5 h-full">
      ...
      <Flex direction="column" maw={1440} mx={'auto'} gap={{base: 40, md: 60, lg: 80}} px={{base: 20, md: 40, lg: 60}} pt={{base: 40, md: 80}} pb={{base: 20, md: 40}}>
        <Flex direction={'column'} gap="md">
          ...
          <Text fz={'md'} ta="center">
            {'A focused set of services to help you design, build, and scale reliable software.'}
          </Text>
        </Flex>
        <Grid gap={"xl"}>
          {
