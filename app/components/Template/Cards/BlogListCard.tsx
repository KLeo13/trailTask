...
const BlogListCard: React.FC<WritingItemsProps> = (props) => {
    ...
    return (
        <>
            <Card p={{base: 'sm', sm: 24}} bg={computedColorScheme === 'dark' ? theme.colors.blue[8] : 'white'} bdrs={'md'} bd={`solid 1px ${computedColorScheme === 'dark' ? theme.colors.blue[7] : theme.colors.blue[1]}`}
                ...
                <Flex gap={{base: 'sm', sm: 40}} align={'center'}>
                    <Image src={props.image} w={{base: 100, sm: 286, md: 357}} h={{base: 100, sm: 185, md:230}} alt={props.image} bdrs={'md'}/>
                    <Stack>
                        ...
                        <Link to={`/blogs/${props.slug}`}><Text fz={{base: 'md', sm: 24}} fw={"bold"} c={computedColorScheme === 'dark' ? theme.colors.blue[4] : theme.colors.blue[5]} lineClamp={1}>{props.title}</Text></Link>
